...
int main()
{
    ...
    while (!cin.eof())
    {
        ...
        if (!line.empty())
        {
            ...
            {
                ReadRectangle(elems, figures);
            }
            else if (elems[0] == "triangle")
            {
                ReadTriangle(elems, figures);
            }
            else if (elems[0] == "circle")
            {
                ReadCircle(elems, figures);
            }
            else if (elems[0] == "line")
            {
                ReadLine(elems, figures);
            }
        }
    }
    int indexS = BiggestAreaIndex(figures);
    int indexP = SmallestPerimeterIndex(figures);

    cout << "Наибольшая площадь - " << indexS + 1 << " фигуры" << endl;
    GiveShapeInfo(*figures[indexS]);
    cout << endl;

    cout << "Наименьший периметр - " << indexP + 1 << " фигуры" << endl;
    GiveShapeInfo(*figures[indexP]);
}
